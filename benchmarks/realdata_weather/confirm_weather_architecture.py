#!/usr/bin/env python3
"""Virgin Weather confirmation of the active experimental architecture (ACI).

Protocol: WEATHER_ARCHITECTURE_CONFIRMATION_PROTOCOL.md
Subject: ActiveExperimentalCellular — not Wave XI.
One look on confirmation2. No retuning.
"""
...
def gate(aci: dict, silence: dict, persist: dict) -> dict:
    checks = {
        "flip_ge_0.45": aci["flip_accuracy"] >= FLIP_MIN,
        "flip_ge_silence_lineage": aci["flip_accuracy"] >= silence["flip_accuracy"],
...
def main() -> None:
    data = ROOT / "data_confirmation2"
    stream = CleanWeatherStream(data)
    # Disjointness vs contact bed.
    contact_meta = json.loads((ROOT / "data" / "download_meta.json").read_text())
    contact_cities = {city["symbol"] for city in contact_meta["cities"]}
    conf_cities = set(stream.city_symbols)
    if contact_cities & conf_cities:
        raise RuntimeError(f"confirmation2 overlaps contact bed: {contact_cities & conf_cities}")
    ...
    decision = gate(aci, silence, persistence)

    payload = {
        "protocol": "weather_architecture_confirmation_v1",
        "architecture_under_test": "ActiveExperimentalCellular (ACI)",
        "not_under_test": "BatchedReserveCellular (Wave XI synthetic reference)",
        "candidate": f"CleanWeatherStream + ActiveExperimentalCellular({ACI_FROZEN})",
        ...
        "paired": {
            "vs_silence_accuracy": paired_delta(
                silence["correct"], aci["correct"], n_boot=5000
            ),
            "vs_silence_flip": paired_delta(
                silence["correct"], aci["correct"], silence["flip"], n_boot=5000
            ),
    ...
    path = out / "weather_architecture_confirmation.json"
    path.write_text(json.dumps(payload, indent=2, default=float))

    print("WEATHER ARCHITECTURE CONFIRMATION (ACI, not Wave XI)")
