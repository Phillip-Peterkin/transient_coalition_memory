# Diagnostic-contrast confirmation protocol

Written **before** scoring the fifth fresh company universe.
...
4. Candidate: `tcm.DiagnosticContrastCellular` with frozen hyperparameters
   - same silence-escape PE knobs as above
   - `slot_preserve_scale=0.50`
   - `use_slot_null=True`
   - `use_survival_hazard=False`
...
`confirmation5_universe.py` declares 30 companies disjoint from all earlier
company sets. Selected from curated liquid names using only in-window news-row
availability (≥35 sentiment-labelled rows). Ambiguous headline-token tickers
excluded.
...
- Compare diagnostic-contrast vs clean evidence and vs silence-escape on
  identical session events.
...
Pass or fail is recorded once. No retuning on this universe. A pass does not
require beating silence-escape, but the silence comparison is reported for
honesty about whether the DCAI bake added sealed lift.
