#!/usr/bin/env python3
"""One-shot fresh-company confirmation for frozen diagnostic contrast.

Protocol: DIAGNOSTIC_CONTRAST_CONFIRMATION_PROTOCOL.md
Universe: confirmation5 (virgin). One look. No retuning.
"""
...
def main() -> None:
    ...
    ap.add_argument("--data-dir", type=Path, default=ROOT / "data_confirmation5")
    ap.add_argument(
        "--out",
        type=Path,
        default=ROOT / "results" / "diagnostic_contrast_confirmation.json",
    )
    ...
    if candidate["n"] != clean["n"] or not (candidate["flip"] == clean["flip"]).all():
        raise RuntimeError("diagnostic-contrast candidate no longer aligns with clean")
    ...
    payload = {
        "protocol": "diagnostic_contrast_confirmation_v1",
        ...
        "universe": "confirmation5_universe",
        ...
        "confirmation": {
            ...
            "diagnostic_contrast": concise(candidate),
            "paired": comparison,
    ...
    for name, result in (
        ...
        print(
            f"{name:12s} acc={result['accuracy']:.4f} flip={result['flip_accuracy']:.4f} "
