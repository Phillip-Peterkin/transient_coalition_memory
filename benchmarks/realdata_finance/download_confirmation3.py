#!/usr/bin/env python3
"""Cache the third untouched company universe for skew-correction confirmation."""
...
from confirmation3_universe import CONFIRMATION3_UNIVERSE
...
def main() -> None:
    ...
    if args.force or not news_path.exists():
        news = fetch_news(CONFIRMATION3_UNIVERSE)
    ...
    meta = {
        "role": "untouched_skew_confirmation",
        "news_dataset": NEWS_DATASET,
        "selection": (
            "disjoint fixed universe; curated liquid US equities; "
            "selected from raw news-row coverage only"
        ),
        "declared_symbols": CONFIRMATION3_UNIVERSE,
